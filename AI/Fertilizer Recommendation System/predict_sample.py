...
def analyze_prediction(fertilizer_key: str, crop_name: str, growth_stage: str, N: float, P: float, K: float, ph: float, temperature: float):
    ...
    return main_problem, explanation
def predict_fertilizer(N, P, K, temperature, humidity, ph, month, soil_type, governorate, crop_name, growth_stage):
    sample, region, season = build_input_sample(N, P, K, temperature, humidity, ph, month, soil_type, governorate, crop_name, growth_stage)
    ...
    if hasattr(model, "predict_proba"):
        try:
            ...
            confidence = 1.0
    risk_score = 0
    if temperature >= 38: risk_score += 2

    if ph >= 8.0 or ph <= 5.8: risk_score += 2

    if N > 1.2 * prof.optimal_N[1] or P > 1.2 * prof.optimal_P[1] or K > 1.2 * prof.optimal_K[1]: risk_score += 2

    if soil_type in ["sandy", "reclaimed"] and growth_stage == "seedling": risk_score += 2

    is_starving = N < prof.optimal_N[0] * 0.75 or P < prof.optimal_P[0] * 0.75 or K < prof.optimal_K[0] * 0.75
    is_lazy_ml = (fertilizer_key == "no_fertilizer_required")

    apply_rules = (risk_score >= 2) or (confidence < 0.55) or is_starving or is_lazy_ml


    if N > prof.optimal_N[1] * 1.25 or P > prof.optimal_P[1] * 1.25 or K > prof.optimal_K[1] * 1.25:
        fertilizer_key = "nutrient_excess"
        apply_rules = False 

    if apply_rules:
        if fertilizer_key == "urea_46" and temperature > 35:
            fertilizer_key = "calcium_nitrate_155"

  
        elif fertilizer_key == "potassium_chloride_60" and (spec.chloride_sensitive or soil_type in ["sandy", "reclaimed"]):
            fertilizer_key = "potassium_sulphate_48"

        elif N < prof.optimal_N[0] * 0.70 and P < prof.optimal_P[0] * 0.70 and K < prof.optimal_K[0] * 0.70:
            fertilizer_key = "npk_20_20_20" if crop_name in heavy_feeders else "npk_19_19_19"

        elif P < prof.optimal_P[0] * 0.85 and ph >= 7.8 and fertilizer_key not in ["phosphoric_acid_85", "mono_ammonium_phosphate_12_61", "di_ammonium_phosphate_18_46"]:
            fertilizer_key = "phosphoric_acid_85"

   
        elif fertilizer_key == "no_fertilizer_required" or risk_score > 0:
            if growth_stage == "seedling" and soil_type in ["sandy", "reclaimed"]:
                fertilizer_key = "humic_acid_80"
            elif spec.boron_sensitive and growth_stage in ["flowering", "fruiting"]:
                fertilizer_key = "borax_11"
            elif ph >= 8.2 and spec.iron_sensitive:
                fertilizer_key = "iron_chelate_eddha_6"
            elif ph >= 8.0 and spec.zinc_sensitive and growth_stage == "vegetative":
                fertilizer_key = "zinc_sulphate_22"
            elif ph >= 8.2 and growth_stage in ["seedling", "vegetative"] and soil_type in ["calcareous", "reclaimed"]:
                fertilizer_key = "manganese_sulphate_32"

    main_problem, explanation = analyze_prediction(
        fertilizer_key, crop_name, growth_stage, N, P, K, ph, temperature
    )
    ...
    return {
        ...
        "recommended_fertilizer_key": fertilizer_key,
        "main_problem": main_problem,
        "explanation": explanation,
    }

def format_result(result: dict) -> str:
    fert_key = result['recommended_fertilizer_key'].lower()
    if fert_key in ["no_fertilizer_required", "nutrient_excess"]: app_method = "N/A (Farm Management / Water Irrigation)"
    elif any(x in fert_key for x in ["chelate", "zinc", "manganese", "molybdate", "borax"]): app_method = "Foliar Spray or Fertigation"
    else: app_method = "Soil Application or Fertigation"

    explanation_lines = "\n".join([f"      • {line}" for line in result['explanation']])
    return f" Detected Issue : {result['main_problem']}\n Recommendation : {result['recommended_fertilizer_key'].upper()}\n App Method : {app_method}\n Why this choice?\n{explanation_lines}\n"
