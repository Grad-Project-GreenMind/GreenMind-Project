...
const Profile = () => {
  ...
  // لتخزين ملف الصورة الحقيقي لإرساله للسيرفر
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.log("Status Code:", err.response?.status); // هيطبع 403
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      // 1. استخدام FormData لإرسال الصورة كملف حقيقي[cite: 1]
      ...
      if (selectedFile) {
        formData.append("profilePic", selectedFile); // إرسال الملف[cite: 1]
      }
      // else {
      //   if (userData.profilePic && !userData.profilePic.startsWith("blob")) {
      //     formData.append("profilePic", userData.profilePic);
      //   }
      // }
      // إذا لم يتم تغيير الصورة، نبعت الرابط القديم كـ string عادي
      //   formData.append("profilePic", userData.profilePic);
      // }

      const response = await axios.put(API_ENDPOINTS.UPDATE_PROFILE, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          // "Content-Type": "multipart/form-data", // مهم جداً لاستقبال الملفات في الباك[cite: 1]
          "ngrok-skip-browser-warning": "69420",
        },
      });
    ...
    <div className="bg-[#F9FFF9] font-inter flex flex-col text-[#432818] w-full min-h-screen">
      <main className="flex-grow w-full max-w-7xl mx-auto px-8 md:px-20 py-12 flex flex-col items-center">
        ...
        <div className="flex flex-col items-center relative w-full max-w-2xl">
          ...
          {error && (
            <p className="text-red-500 mb-5 font-bold animate-pulse text-center w-full">
              ⚠️ {error}
            </p>
          )}
          ...
          <div className="w-full space-y-10">
            {[
              ...
              >
                <label className="text-[24px] md:text-[28px] font-[700] text-[#432818]">
                  {field.label}
                </label>
                {!isEditMode || field.disabled ? (
                  <div className="w-full p-4 rounded-[15px] border border-gray-300 bg-white text-[18px] flex items-center min-h-[60px]">
            ...
            <div className="flex items-center justify-center gap-8 pt-4 text-[18px] font-semibold text-[#432818]">
              ...
              <div
                ...
                onClick={() =>
                  isEditMode && setUserData({ ...userData, gender: "Male" })
                }
              >
                <div className="w-5 h-5 rounded-full border-2 border-[#432818] flex items-center justify-center">
                  {userData.gender === "Male" && (
                    <div className="w-2.5 h-2.5 rounded-full bg-[#432818]"></div>
                  )}
                </div>
                <span>Male</span>
              </div>
              <div
                className={`flex items-center gap-2 ${isEditMode ? "cursor-pointer" : "cursor-default"}`}
                onClick={() =>
                  isEditMode && setUserData({ ...userData, gender: "Female" })
                }
              >
                <div className="w-5 h-5 rounded-full border-2 border-[#432818] flex items-center justify-center">
