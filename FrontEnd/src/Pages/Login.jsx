...
const Login = () => {
  ...
  // تحديد الوجهة: يرجع للمكان اللي جه منه (from) أو يروح للهوم/الداش بورد كافتراضي
  const from =
    location.state?.from ||
    (selectedRole === "admin" ? "/home-dashboard" : "/");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        // localStorage.setItem("userId", response.data.userId || "1"); // لو مجاش من الباك حط 1 مؤقتاً
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1400px] px-8 md:px-16 flex flex-col md:flex-row items-center justify-between min-h-screen py-10">
        ...
        <div className="flex-[0.6] w-full flex justify-end items-center">
          <div
            className="w-full max-w-[650px] py-8 md:py-10 px-6 md:px-10 flex flex-col items-center shadow-2xl transition-all animate-in fade-in duration-500"
            style={{
          ...
          >
            <div className="flex items-center gap-4 mb-7">
              ...
              <h3 className="text-white text-[28px] font-bold tracking-wide">
                GreenMindAI
              </h3>
            </div>
            <form
            ...
            >
              ...
              <div className="flex gap-10 my-6 text-white text-[18px] font-normal">
                <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span
                    className={
                      selectedRole === "user" ? "font-bold text-[#4CAF50]" : ""
                    }
                  >
                    User
                  </span>
                ...
                <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span
                    className={
                      selectedRole === "admin" ? "font-bold text-[#4CAF50]" : ""
                    }
                  >
                    Admin
                  </span>
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-[70%] text-white font-bold py-3 text-[18px] shadow-lg transition-all mt-4 transform active:scale-95 shadow-[#4CAF50]/20 ${loading ? "opacity-50 cursor-not-allowed" : "hover:brightness-110 cursor-pointer"}`}
            ...
            <div className="w-full flex items-center my-6">
              ...
              <span className="px-3 text-white/50 text-[12px] uppercase tracking-widest font-medium">
                Or Log in with
              </span>
              <div className="flex-grow border-t border-white/20"></div>
            </div>
            ...
            <p className="text-white/70 text-[14px] font-normal">
              ...
              <button
                ...
                className="font-bold text-white underline italic ml-1 cursor-pointer"
              >
                Create Now
              </button>
            </p>
